...
async function loadStoredState() {
  ...
  try {
    ...
    settings.modelName = normalizeModelName(settings.modelName);
    conversationHistory = sanitizeChatHistory(JSON.parse(localStorage.getItem("trump-ai-chat-history") || "[]"));
    addMessage("system", `Database fallback active: ${error.message}`);
  }
}
...
// Turns a small, safe subset of Markdown (bold, italics, bullet/numbered lists,
// paragraphs) into HTML for display. Everything is escaped first, so this never
// introduces real HTML/script injection — it only recognizes literal ** and - / 1.
// patterns and wraps them in tags.
...
function exportChatHistory() {
  ...
  addMessage("system", "Chat exported as a Markdown file.");
}
...
async function runAutomationCheck() {
  ...
  if (!activeTasks.length) {
    addMessage("system", `Automation check completed at ${checkedAt}. No active alerts are turned on.`);
    return;
  }
...
function renderReadiness() {
  ...
  const checks = [
    {
      label: "Backend API",
      detail: backendStatus.checked
        ? (backendStatus.ok ? "Connected to the backend." : "Could not reach the backend. Check your connection or try again shortly.")
        : "Checking backend connection...",
      state: backendStatus.checked ? (backendStatus.ok ? "ready" : "missing") : "partial",
    },
    {
      label: "AI API",
      detail: backendStatus.checked
        ? (backendStatus.aiConfigured ? "AI chat is connected and ready." : "AI chat setup is still pending on the backend.")
        : "Checking AI chat availability...",
      state: backendStatus.checked ? (backendStatus.aiConfigured ? "ready" : "missing") : "partial",
    },
...
document.querySelector("#refresh-button").addEventListener("click", () => {
  refreshBrief();
  addMessage("system", "Briefings refreshed. I updated the dashboard snapshot and kept your saved automations intact.");
});
...
document.querySelector("#export-tasks-button").addEventListener("click", async () => {
  ...
  if (navigator.clipboard) {
    try {
      ...
      addMessage("system", "I copied the current settings and automations as JSON so they are ready to wire into a backend or scheduler.");
      return;
    } catch {
...
document.querySelector("#reset-tasks-button").addEventListener("click", async () => {
  ...
  renderAlertSummary();
  renderReadiness();
  addMessage("system", "Automation defaults restored.");
});
document.querySelector("#save-settings-button").addEventListener("click", async () => {
  ...
  renderReadiness();
  renderIntegrationPlan();
  addMessage("system", "Settings saved. The browser will call your backend for live data and AI requests.");
});

async function checkBackendStatus() {
  try {
    const data = await fetchBackendJson("/api/health");
    backendStatus = { checked: true, ok: Boolean(data.ok), aiConfigured: Boolean(data.aiConfigured) };
  } catch (error) {
    backendStatus = { checked: true, ok: false, aiConfigured: false };
  }
  renderReadiness();
}
